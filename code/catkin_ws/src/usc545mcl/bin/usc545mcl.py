...
class Particle(object):
    ...
    def UpdateOdom(self, odom_msg):
        '''Propagate this particle according to the sensor data and motion model.'''
    ...
    def UpdateScan(self, scan):
        ...
        #  Compute the likelihood of each beam, compared to sim_ranges
        probs = []
        for i in range(len(sim_ranges)):
            ...
            probs.append(p)
        #  Assign the weight of this particle
        prod = 1
        for i in probs:
            prod *= i
        self.weight = prod
...
class ParticleFilter(object):
    ...
    def UpdateScan(self, scan_msg):
        ...
        #  Use these weights to build a normalized discrete probability distribution for sampling each particle
        norm_weights = weights / np.sum(weights)
        #  Randomly choose the indices of the particles to propagate
        select_indices = np.random.choice(NUM_PARTICLES, size=NUM_PARTICLES, p=norm_weights)
        #  Copy the proper particles into a new population
        select_particles = [copy.deepcopy(self.particles[i]) for i in select_indices]
        #  Set self.particles to the new population of particles.
